...
import com.example.gate360player.gles.EGLRenderTarget;
import com.example.gate360player.gles.GLHelpers;
import com.example.gate360player.gles.SphericalSceneRenderer;
...
public class SphericalVideoPlayer extends TextureView {
    ...
    private boolean playStaticVideo;
    ...
    public void setPlayStaticVideo(boolean playStaticVideo) {
        this.playStaticVideo = playStaticVideo;
    }
    ...
    private void prepareVideo(String videoPath) {
        ...
        if (TextUtils.isEmpty(videoPath) && (!playStaticVideo)) {
            return;
        ...
        try {
            ...
            if (playStaticVideo){
                ...
                //Play static videox
                //AssetFileDescriptor afd = getContext().getResources().openRawResourceFd(R.raw.scrum);
                //videoPlayerInternal.setDataSource(afd.getFileDescriptor(), afd.getStartOffset(), afd.getLength());
                //afd.close();
            }else {
                videoPlayerInternal.setDataSource(videoPath);
            }
            ...
            videoPlayerInternal.setOnErrorListener(new MediaPlayer.OnErrorListener() {
                @Override
                public boolean onError(MediaPlayer mp, int what, int extra) {
                    //toast(getContext(), "onError");
                    return false;
                }
            });
    ...
    public void prepareWebRTCPlayer(){
        renderThread.getVideoDecodeSurface();
        renderThread.handler.sendEmptyMessage(RenderThread.MSG_FRAME_AVAILABLE);
    }
